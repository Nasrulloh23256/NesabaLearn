...
const PickerField = ({ id, label, name, type, value, placeholder, required, onChange }) => {
  ...
  return (
    <div>
      ...
      <div className="relative group rounded-2xl" onClick={openPicker}>
        <input
          id={inputId}
          name={name}
          type={type}
        ...
        />
        <div
          className={`${pickerBase} ${value ? "" : "text-slate-400 dark:text-slate-500"} group-focus-within:ring-2 group-focus-within:ring-[#04BBFD] group-focus-within:ring-offset-2 group-focus-within:ring-offset-slate-50 dark:group-focus-within:ring-offset-slate-900`}
        >
...
export default function MeetingCreatePage() {
  ...
  const formCopy = meetingForm.create;
  const { data, setData, post, processing, errors } = useForm({
    title: "",
    type: meetingTypeOptions[0].value,
  ...
  const attachmentStatus = attachments.length
    ? meetingForm.filePicker.count.replace("{{count}}", attachments.length)
    : meetingForm.filePicker.empty;
    ...
    <AdminLayout>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 md:p-8 shadow-sm dark:border-slate-700/60 dark:bg-slate-900/70">
          ...
          <div className="mt-5">
            ...
            <div className="rounded-3xl border border-dashed border-slate-200 bg-slate-50/80 p-5 text-center dark:border-slate-700 dark:bg-slate-900/40">
              ...
              <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">{attachmentStatus}</p>
        ...
        <div className="flex flex-wrap justify-end gap-3">
          ...
          <button
            type="submit"
            className="group relative overflow-hidden rounded-full bg-gradient-to-r from-[#04BBFD] to-[#FB00FF] px-8 py-2.5 text-sm font-semibold text-white shadow-lg transition hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-[#04BBFD]/60 focus:ring-offset-2 focus:ring-offset-slate-50 disabled:opacity-60 dark:focus:ring-offset-slate-900"
            disabled={processing}
          >
            {formCopy.submit}
          </button>
