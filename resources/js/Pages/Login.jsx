...
export default function Login() {
  ...
  useEffect(() => {
    const lastSignup = window.localStorage.getItem("nesabaLastSignup");
    if (lastSignup) {
      setForm((prev) => ({ ...prev, email: lastSignup }));
    }
  }, []);
  ...
  const handleSubmit = (event) => {
    ...
    const users = JSON.parse(window.localStorage.getItem("nesabaUsers") || "[]");
    ...
    window.localStorage.setItem("nesabaActiveUser", JSON.stringify(match));
    window.localStorage.removeItem("nesabaLastSignup");
    setSuccess("Login berhasil, mengalihkan ke dashboard...");
    setForm({ email, password: "" });

    const destination = match.role === "Admin" ? "/admin" : "/user/meetings";
...
<div className="hidden md:flex md:w-1/2 h-screen relative overflow-hidden">
  ...
  <div className="flex justify-between items-center mb-2">
    ...
    <Link
      to="/forgot-password"
      className="relative text-sm font-semibold bg-gradient-to-r from-[#04BBFD] to-[#FB00FF] bg-clip-text text-transparent
                 after:content-[''] after:block after:w-full after:h-[2px] after:bg-gray-400/50 
                 after:transition-all after:duration-300 after:absolute after:left-0 after:-bottom-1
                 hover:after:bg-gradient-to-r hover:after:from-[#04BBFD] hover:after:to-[#FB00FF] 
                 hover:opacity-80"
    >
